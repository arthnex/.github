<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=3D3580&height=120&section=header&text=Arthnex&fontSize=42&fontColor=ffffff&fontAlignY=38&desc=Engineer%20the%20Future&descAlignY=62&descSize=16" width="100%"/>

</div>

<div align="center">

<br>

> **We don't just write code. We engineer futures.**

A software studio at the intersection of craft and ambition — building digital products, SaaS platforms, and tools that people actually love using.

<br>

![](https://img.shields.io/badge/Founded-2024-6C63FF?style=flat-square&labelColor=0B0B12)
![](https://img.shields.io/badge/Based_in-India-00D4AA?style=flat-square&labelColor=0B0B12)
![](https://img.shields.io/badge/Remote-First-6C63FF?style=flat-square&labelColor=0B0B12)
![](https://img.shields.io/badge/Status-Building-00D4AA?style=flat-square&labelColor=0B0B12)

</div>

<br>

<img src="https://i.imgur.com/dBaSKWF.gif" height="2" width="100%">

<br>

## 🚀 &nbsp;What We're Building

<br>

<table width="100%">
  <tr>
    <td width="50%" valign="top">
      <h3>🧩 ScreenerLens</h3>
      <p>A Chrome Extension that enhances how users research, screen, and evaluate content directly in their browser. Built for speed, designed for focus.</p>
      <p>
        <img src="https://img.shields.io/badge/Status-Live-34D399?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/Chrome_Extension-6C63FF?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&labelColor=0B0B12"/>
      </p>
    </td>
    <td width="50%" valign="top">
      <h3>🎓 ClassMitra</h3>
      <p>The digital operating system for India's local tuition ecosystem — streamlining homework, attendance, fees, and progress tracking while keeping teachers, parents, and students in sync.</p>
      <p>
        <img src="https://img.shields.io/badge/Status-Coming_Soon-6C63FF?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/Mobile_App-00D4AA?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/React_Native-61DAFB?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/Education-00D4AA?style=flat-square&labelColor=0B0B12"/>
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>📈 ArthGrow</h3>
      <p>A personal finance application designed around Indian savings patterns — smart, simple, and built for how real people think about money.</p>
      <p>
        <img src="https://img.shields.io/badge/Status-Upcoming-6C63FF?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/Mobile_App-00D4AA?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/React_Native-61DAFB?style=flat-square&labelColor=0B0B12"/>
      </p>
    </td>
    <td width="50%" valign="top">
      <h3>💍 ShaadiVerse</h3>
      <p>A digital platform reimagining the wedding experience — connecting families, vendors, and moments in one beautiful ecosystem.</p>
      <p>
        <img src="https://img.shields.io/badge/Status-In_Development-FBBF24?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/Web_App-6C63FF?style=flat-square&labelColor=0B0B12"/>
        <img src="https://img.shields.io/badge/Next.js-ffffff?style=flat-square&labelColor=0B0B12"/>
      </p>
    </td>
  </tr>
</table>

<br>

<img src="https://i.imgur.com/dBaSKWF.gif" height="2" width="100%">

<br>

## 🛠️ &nbsp;Our Tech Stack

<br>

**Frontend & Web**
<p>
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,tailwind" />
  </a>
</p>

**Mobile**
<p>
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=react,flutter,dart" />
  </a>
</p>

**Backend & Infrastructure**
<p>
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=nodejs,firebase,supabase,vercel" />
  </a>
</p>

**Design & Tooling**
<p>
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=figma,git,github,vscode" />
  </a>
</p>

<br>

<img src="https://i.imgur.com/dBaSKWF.gif" height="2" width="100%">

<br>

## 💡 &nbsp;How We Think

<br>

```
01  Architecture is never an afterthought     →  Hard decisions made early, always
02  Design and engineering are one discipline →  Pixels and logic are not separate concerns
03  Performance is a feature, not a setting   →  We obsess over speed and latency
04  Ship embarrassingly small, then grow      →  Focused experiments over bloated MVPs
05  Real users over imagined ones             →  No feature ships without external validation
06  Boring technology, exciting outcomes      →  Proven tools. Exceptional results.
```

<br>

<img src="https://i.imgur.com/dBaSKWF.gif" height="2" width="100%">

<br>

## 🤝 &nbsp;Get in Touch

<br>

<div align="center">

[![Email](https://img.shields.io/badge/arthnex.tech@gmail.com-6C63FF?style=for-the-badge&logo=gmail&logoColor=white)](mailto:arthnex.tech@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/company/arthnex)
[![Website](https://img.shields.io/badge/arthnex--website.vercel.app-00D4AA?style=for-the-badge&logo=safari&logoColor=white)](https://arthnex-website.vercel.app/)

</div>

<br>

<img src="https://i.imgur.com/dBaSKWF.gif" height="2" width="100%">

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=008060&height=100&section=footer&text=Building%20what%20matters.&fontSize=16&fontColor=ffffff&fontAlignY=65" width="100%"/>

</div>
